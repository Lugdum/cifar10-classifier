...
def train_and_evaluate_model(
    ...
    if use_hyperparameter_search:
        ...
        random_search = RandomizedSearchCV(
            model, param_distributions, n_iter=10, cv=3, verbose=1, n_jobs=-1, random_state=42
        )
...
def run_training(
    processed_dir="data/processed", use_hyperparameter_search=False, subset_size=None
):
    """
    ...
    classifiers = {
        "LogisticRegression": {
            ...
            "param_distributions": {"C": np.logspace(-4, 4, 20), "solver": ["liblinear", "saga"]},
        },
        "NaiveBayes": {
            "model_class": GaussianNB,
            "model": GaussianNB(),
            "param_distributions": {"var_smoothing": np.logspace(-9, -1, 20)},
        },
        "SVM": {
            "model_class": SVC,
            "model": SVC(),
    ...
    for method in methods:
        try:
            x_train, y_train, x_test, y_test = load_data(
                processed_dir, method, subset_size=subset_size
            )
            for clf_name, clf_info in classifiers.items():
                best_params = train_and_evaluate_model(
                    clf_info["model"],
                    clf_info["param_distributions"],
                    x_train,
                    y_train,
                    x_test,
                    y_test,
                ...
                if use_hyperparameter_search:
                    ...
                    x_train_full, y_train_full, x_test_full, y_test_full = load_data(
                        processed_dir, method
                    )
                    re_train_with_best_params(
                        clf_info["model_class"],
                        best_params,
                        x_train_full,
                        y_train_full,
                        clf_name,
        ...
        try:
            ...
            for clf_name, clf_class in classifiers.items():
                ...
                if os.path.exists(model_path):
                    ...
                    re_train_with_best_params(
                        clf_class, best_params, x_train, y_train, clf_name, method
                    )
                else:
