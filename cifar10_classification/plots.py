...
def plot_combined_roc_curves(models, x_test, y_test, data_type, output_dir=None):
    ...
    if output_dir:
        ...
        logging.info(
            f"ROC curves saved to {os.path.join(output_dir, f'combined_roc_curves_{data_type}.png')}"
        )
        plt.close()
    else:
        plt.show()
...
def plot_combined_confusion_matrices(models, x_test, y_test, data_type, output_dir=None):
    ...
    if output_dir:
        ...
        logging.info(
            f"Confusion matrices saved to {os.path.join(output_dir, f'combined_confusion_matrices_{data_type}.png')}"
        )
        plt.close()
    else:
        plt.show()
...
def main(
    processed_dir=PROCESSED_DATA_DIR, models_dir=MODELS_DIR, output_dir=FIGURES_DIR, final=True
):
    """
...
def evaluate_single_model(model_path, processed_dir=PROCESSED_DATA_DIR, output_dir=FIGURES_DIR):
    ...
    if output_dir:
        ...
        logging.info(
            f"Evaluation plot saved to {os.path.join(output_dir, f'evaluation_{model_name}_{data_type}.png')}"
        )
        plt.close()
    else:
        plt.show()
