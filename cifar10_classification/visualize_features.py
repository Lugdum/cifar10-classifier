...
def visualize_hog(hog_features, y_train, output_dir=FIGURES_DIR, save=False):
    ...
    for i in range(5):
        ...
        if width * height != hog_feature_length:
            print(f'Warning: HOG feature length {hog_feature_length} does not match expected size of {width * height}')
            continue
...
def main(processed_dir=PROCESSED_DATA_DIR, output_dir=FIGURES_DIR, save=False):
    ...
    explained_variance_ratio = np.load(os.path.join(processed_dir, 'pca', 'train', 'explained_variance_ratio.npy'))
